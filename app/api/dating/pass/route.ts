...
export async function POST(req: NextRequest) {
  ...
  if (parsed.data.target_user_id === user.id) return jsonError('You cannot pass your own profile.', 400)

  const serviceClient = createServiceClient()
  const { error } = await serviceClient
    .from('profile_passes')
    .upsert(
      { passer_user_id: user.id, passed_user_id: parsed.data.target_user_id },
